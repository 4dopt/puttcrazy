...
// This prevents html2canvas from crashing on modern oklch color values used in Tailwind v4.
function oklchToRgb(l: number, c: number, h: number, a: number = 1): string {
  ...
  const l_cube = Math.pow(l_lms, 3);
  const m_cube = Math.pow(m_lms, 3);
  const s_cube = Math.pow(s_lms, 3);
...
export default function App() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // Convert OKLCH to RGB in all loaded stylesheets temporarily so html2canvas doesn't crash
